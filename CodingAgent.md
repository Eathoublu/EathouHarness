...
1. 读取 coding_task.md
2. 按任务顺序实现代码
3. 每完成一个任务更新 coding_task.md 中的 `[ ]` 为 `[x]`（每完成一个任务均要标记一次，而不是等到最后才标记）
4. 所有coding_task.md中的任务均完成且标记后，生成 code_files.json
5. 所有coding_task.md中的任务均完成且标记后，生成 `.complete` 信号
